...
namespace dog_breed_world.Controllers
{
  ...
  public class Register_Login : ControllerBase
  {
    ...
    public IResult RegisterAdmin(User user)
    {
    ...
    public IResult Login(UserLogin user)
    {
      try
      {
        if (!string.IsNullOrEmpty(user.Username) && !string.IsNullOrEmpty(user.Password))
        {
          ...
          User? loggedInUser = _context.Users.FirstOrDefault(x => x.Username == user.Username);
          if (loggedInUser is null)
          {
            return Results.Problem("No user found", "login instance", 403);
          }
          if(loggedInUser.Hash != Crypto.Hash(user.Password + loggedInUser.Salt))
          {
            return Results.Problem("Incorrect password", "login instance", 403);
          }
      ...
      {
        return Results.Problem($"Error: {ex.Message} ", "login instance", 500);
      }
